from utils2 import http_get, http_post, get_units_dict, aiohttp_post
import aiohttp
import json
import sys
from os import path
...
async def ai_get_access_token(config):
    ...
    async with aiohttp.ClientSession() as session:
        response, error = await aiohttp_post(session,auth_url, headers=headers, data=data)
        if error:
...
async def get_headers(config):
    token = await ai_get_access_token(config)
    return {
        'method': 'post',
...
#		print("CALL API: %s %s %s" % (url, headers, project), file=sys.stderr)
    ...
    async with aiohttp.ClientSession() as session:
        response, error = await aiohttp_post(session,url, headers=headers,json=project)
        if error:
...
async def api_swgoh_players(config, project):
